<template>
  <div class="container"> 
    ...
    <div>
        ...
        <div class="title">物流管理系统 - 注册</div>
      </div>

    <el-form :model="ruleForm" :rules="rules" ref="ruleForm"   label-width="100px" class="demo-ruleForm">
    <el-form-item class="white-text-button" label="邮箱" prop="LoginName">
    <el-input type="text" v-model="ruleForm.LoginName" placeholder="请输入邮箱" prefix-icon="el-icon-message" autocomplete="off"
    ></el-input>
  </el-form-item>
  <el-form-item class="white-text-button" label="密码" prop="password">
  ...
  <div style="display: flex">
    <el-form-item class="white-text-button" label="验证码" prop="checkPass">
            <el-input type="password" v-model="ruleForm.code" autocomplete="off" placeholder="请确认验证码" prefix-icon="el-icon-lock"></el-input>
  </el-form-item>
  ...
  <div class="btn-group">
  <el-form-item>
    <el-button class="white-text-button" type="primary" @click="submitForm('ruleForm')">注册</el-button>
  ...
  <script>
  import { AdminRegist,AdminSendEmail} from "@/api/admin";
  export default{
    name:"Register",

    mounted() {
      // 在组件挂载之后，设置 label 的字体颜色
      const labelElements = document.querySelectorAll(".demo-ruleForm label");
      labelElements.forEach((label) => {
        label.style.color = "#ffffff"; // 设置您想要的字体颜色
      });
    },
    ...
    methods: {
      ...
      if (this.checkEmail()) {
        ...
        AdminSendEmail(this.ruleForm.LoginName).then((res) => {
      ...
      if (!emailRegex.test(this.ruleForm.LoginName)) {
...
.container {
  ...
  width: 100vw;
  height: 100vh;
  overflow: hidden; /* 确保所有内容都被包含在视口内 */
}
...
.Regist-box {
  ...
  border-radius: 15px;
  background: rgba(255, 255, 255, 0.2); /* 轻微透明的白色背景 */
  backdrop-filter: blur(15px); /* 背景模糊效果 */
  -webkit-backdrop-filter: blur(10px); /* 针对 Safari 浏览器 */
  box-shadow: 0 4px 6px rgba(0, 0, 0, 0.2); /* 细腻的阴影效果 */
  border: 1px solid rgba(255, 255, 255, 0.25); /* 细边框增强玻璃效果 */
...
.white-text-button{
  color: #FFFFFF !important; /* 设置文本颜色为纯白 */
}
</style>
